...
int MASTER_MODE = std::getenv("MASTER_MODE") ? atoi(std::getenv("MASTER_MODE")) : 0;
int PORT_BASE = 55990;
...
void serverThread() {
    ...
    if (getpeername(new_socket, (struct sockaddr*)&addr, &addr_len) == -1) {
        ...
        int host = local_port - CLIENT_PORT_BASE;
    }
    ...
    if (MASTER_MODE) {
        while (true) {
            ...
            if (file_exists(merge_filename)) {
                ...
                sendDataToServer(PORT_BASE + NEIGHBOUR_HOST_NUMBER, greater_part, get_file_prefix(NEIGHBOUR_HOST_NUMBER, "/file" + std::to_string(NEIGHBOUR_HOST_NUMBER)));    
...
int main(int argc, char* argv[]) {
    ...
    std::cout << HOST_NUMBER << " " << NEIGHBOUR_HOST_NUMBER << " " << MASTER_MODE << std::endl;
